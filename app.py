...
if st.session_state.get("tevhid_aktif"):
    st.info(
        f"🔗 **TEVHİD (ÇOKLU PARSEL) MODU AKTİF** | Ada: `{st.session_state.ada}` | Parsel: `{st.session_state.parsel}` | Toplam Tapu: `{tapu_alani:,.2f} m²`"
    )
...
with tab1:
    ...
    c2.metric(
        "Hesaba Esas Net Alan",
        f"{net_alan:,.2f} m²",
        delta=(
            f"-%{int(kesinti_orani*100)} DOP"
            if kesinti_orani > 0
            else "Kesintisiz"
        ),
    )
...
def yatay_kurumsal_pdf_olustur():
    ...
    table1 = Table(
        [headers_t1, row_t1],
        colWidths=[80, 45, 45, 70, 95, 90, 125, 45, 100, 107],
    )
    table1.setStyle(
        TableStyle([
